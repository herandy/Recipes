...
def train_test(depth, growth_rate, dropout, augment, validate, epochs,
    ...
    input_var_clean = T.tensor4('inputs')
    ...
    prediction_no_aug = lasagne.layers.get_output(network, input_var_clean, deterministic=True)
    ...
    loss_aug = lambda_clean * lasagne.objectives.squared_error(prediction, prediction_no_aug).mean()
    params = lasagne.layers.get_all_params(network, trainable=True)
    eta = theano.shared(lasagne.utils.floatX(eta), name='eta')
    updates = lasagne.updates.nesterov_momentum(
            loss + loss_aug + l2_loss, params, learning_rate=eta, momentum=0.9)
    train_fn = theano.function([input_var, input_var_clean, target_var], [loss, loss_aug], updates=updates)
    ...
    for epoch in range(epochs):
        ...
        batches = cifar10.iterate_minibatches(X_train, y_train, batchsize,
                                              shuffle=True)
        if augment:
            batches = cifar10.augment_minibatches_with_clean(batches)
            batches = generate_in_background(batches)
        batches = progress.progress(
                batches, desc='Epoch %d/%d, Batch ' % (epoch + 1, epochs),
                total=train_batches)
        for inputs_clean, inputs, targets in batches:
            tr_loss, cl_loss = train_fn(inputs, inputs_clean, targets)
