...
def build_densenet(input_shape=(None, 3, 32, 32), input_var=None, classes=10,
    ...
    for b in range(num_blocks):
        if b == 0:
            ...
            block_size = n - 2
        network = dense_block(network, block_size, (2**b) * growth_rate, dropout, b, blocks,
                              name_prefix='block%d' % (b + 1))
        if b < num_blocks - 1:
            network = transition(network, dropout, blocks[b],
...
def dense_block(network, num_layers, growth_rate, dropout, block_num, blocks, name_prefix):
    ...
    for n in range(num_layers):
        ...
        if n == 0:
            blocks[block_num] = conv
        else:
            blocks[block_num] = ConcatLayer([blocks[block_num], conv], axis=1,
                                  name=name_prefix + '_l%02d_join' % (n + 1))
        network = conv
    return network


def transition(network, dropout, block, name_prefix):
